...
from src.FaceRecognition.faceNet.faceNetRecognition import findPeople,face_locations_encoding

def start(EncodingCache,face_encodings):
    '''判断是否开始判断人脸'''
    if len(face_encodings)<1:
        return False
    if len(EncodingCache)<5:
        return False
...
def recognition(frame,EncodingCache,known_face_dataset):
    ...
    face_locations, face_encodings, positions,image = face_locations_encoding(rgb_small_frame)
    ...
    if len(face_locations)>0: #start(EncodingCache, face_encodings):  # 如果达到判断人脸的条件
        ...
        face_names = findPeople(face_encodings,positions,data_set=known_face_dataset)
